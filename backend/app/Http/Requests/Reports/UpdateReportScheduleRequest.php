...
class UpdateReportScheduleRequest extends ReportsRequest
{
    public function rules(): array
    {
        return [
            'report_definition_id' => ['sometimes', 'required', 'integer', $this->existsInTenant('report_definitions')],
            'schedule_type' => ['sometimes', 'required', 'string', Rule::in(['once', 'daily', 'weekly', 'monthly'])],
            'schedule_config' => ['sometimes', 'required', 'array', $this->validScheduleConfig()],
            'schedule_config.time' => ['sometimes', 'required', 'date_format:H:i'],
            ...
            'channel' => ['sometimes', 'required', 'string', Rule::in(['email', 'in_app'])],
            'recipients' => ['sometimes', 'required', 'array', 'min:1', $this->validRecipients()],
            ...
            'status' => ['sometimes', 'required', 'string', Rule::in(['active', 'paused', 'cancelled'])],
        ];
    }
    ...
    protected function validScheduleConfig(): Closure
    {
        return function (string $attribute, mixed $value, Closure $fail): void {
            ...
            $scheduleType = $this->input('schedule_type', $this->route('schedule')?->schedule_type);
