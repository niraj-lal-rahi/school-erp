...
class UpdateReportDefinitionRequest extends ReportsRequest
{
    public function rules(): array
    {
        $definitionId = $this->routeModelId('definition') ?? $this->routeModelId('report_definition') ?? $this->routeModelId('id');

        return [
            'name' => ['sometimes', 'required', 'string', 'max:255'],
            'code' => ['sometimes', 'required', 'string', 'max:80', $this->uniqueInTenant('report_definitions', 'code', $definitionId)],
            'module' => ['sometimes', 'required', 'string', Rule::in(['dashboard', 'attendance', 'finance', 'exams', 'transport', 'communication', 'custom'])],
            'description' => ['nullable', 'string'],
            'query_config' => ['sometimes', 'required', 'array', $this->validQueryConfig()],
