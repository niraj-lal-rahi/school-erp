...
class SecuritySettingService
{
    ...
    {
        $globalVersion = $this->cache->namespaceVersion('security', null);

        return $this->cache->remember(
            'security',
            $schoolId,
            ['record', 'global-version', $globalVersion],
            now()->addMinutes(30),
            fn () => $this->security->findForTenant($schoolId)
        );
    }

    public function update(?int $schoolId, array $attributes, $actor = null, ?Request $request = null): SecuritySetting
    {
        $existing = $this->security->findForTenant($schoolId);
        $security = $this->security->upsert($schoolId, $attributes);
        $this->invalidator->security($schoolId);
    ...
    public function authRules(?int $schoolId = null): array
    {
        ...
        return $this->cache->remember(
            'security',
            $schoolId,
            ['auth-rules', 'global-version', $globalVersion],
            now()->addMinutes(30),
